...
must_haves:
  ...
  key_links:
    ...
    - from: "src/ui/layout/MainPane.tsx"
      ...
      via: "Match branch for filter-* pages renders filtered atom view using savedFilter config"
      pattern: "filter-"
---
  ...
  <action>
    ...
    **2. Verify/update `src/ui/layout/PageTabStrip.tsx`:**
    ...
    - After static tabs + section tabs + review tab, append saved filter tabs:
      ...
      ```
    **3. Update `src/ui/layout/MainPane.tsx`:**

    Add a Match branch for saved filter pages (without this, clicking a saved filter tab renders nothing — NAV-07 is incomplete):
    - Add a `Match when={state.activePage.startsWith('filter-')}` branch
    - Extract filter ID: `state.activePage.replace('filter-', '')`
    - Look up the SavedFilter from `state.savedFilters.find(f => f.id === filterId)`
    - Render a filtered atom list using `filteredAndSortedAtoms(() => state.atoms, () => savedFilter.filter)` from queries.ts
    - Show a page header with the saved filter name
    - Render the atom list with `<For each={filteredAtoms()}>` using AtomCard
    - If the saved filter is not found (deleted), fall back to the default page
