...
must_haves:
  truths:
    ...
    - "Before each cloud request, user sees a preview of the exact data the local LLM will send and can cancel before dispatch"
    - "A communication log section in AI Settings shows a history of cloud requests for user review"
  ...
  artifacts:
    ...
    - path: "src/ui/components/CloudRequestPreview.tsx"
      provides: "Pre-send preview modal showing exact data going to cloud, with cancel option"
      contains: "CloudRequestPreview"
    - path: "src/ui/layout/Shell.tsx"
      provides: "Render site for AISettingsPanel and AIGuidedSetup overlays"
      contains: "AISettingsPanel"
  ...
  key_links:
    ...
    - from: "src/ui/layout/Shell.tsx"
      to: "src/ui/components/AISettingsPanel.tsx"
      via: "Shell conditionally renders AISettingsPanel overlay"
      pattern: "AISettingsPanel"
    - from: "src/ui/layout/Shell.tsx"
      to: "src/ui/components/AIGuidedSetup.tsx"
      via: "Shell conditionally renders AIGuidedSetup when aiFirstRunComplete is false"
      pattern: "AIGuidedSetup"
    - from: "src/ai/adapters/cloud.ts"
      to: "src/ui/components/CloudRequestPreview.tsx"
      via: "Cloud adapter dispatch triggers pre-send preview for user approval before each request"
      pattern: "CloudRequestPreview"
    - from: "src/ui/layout/Shell.tsx"
      to: "src/ai/adapters/cloud.ts"
      via: "Shell.tsx wires cloudAdapter.setPreSendApprovalHandler() in createEffect when cloudAPIEnabled becomes true, setting pendingCloudRequest store state to trigger CloudRequestPreview"
      pattern: "setPreSendApprovalHandler"
---
...
export class CloudAdapter implements AIAdapter {
  ...
  async execute(request: AIRequest): Promise<AIResponse> {
    ...
    const stream = this.client.messages.stream({
      ...
      messages: [{ role: 'user', content: sanitizeForCloud(request.prompt, 'structured') }],
    });
...
**Cloud request log (in `key-vault.ts` alongside consent tracking):**

Per CONTEXT.md: "Communication log accessible in settings for review" and "Full transparency on cloud requests."
...
**Pre-send preview gate in CloudAdapter.execute():**

Per CONTEXT.md locked decision: "every cloud request shows a preview of what the local LLM is sending. User can see exactly what data leaves the device and can cancel before sending."

Update the `CloudAdapter.execute()` method to NOT send directly. Instead, it creates a pending log entry and calls a `requestPreSendApproval` callback:

```typescript
// Add to CloudAdapter constructor or as a module-level setter:
...
In `execute()`, before making the API call:
```typescript
...
// Pre-send preview: user must approve before data leaves device
...
After successful response, update `logEntry.status = 'completed'`. On error, update `logEntry.status = 'error'`.
...
**Install Anthropic SDK:** The executor must run `pnpm add @anthropic-ai/sdk` before building. Add this as the first step in the action.
  ...
  <verify>
Run `pnpm add @anthropic-ai/sdk` then `npx tsc --noEmit` — all files compile. Verify `CloudAdapter` imports `getMemoryKey` and `hasSessionConsent`. Verify `sanitizeForCloud` is exported from privacy-proxy.ts. Verify `encryptAndStore` uses `crypto.subtle` (Web Crypto API). Verify `isDestructiveAction` is exported from cloud.ts. Verify `CloudRequestLogEntry`, `addCloudRequestLog`, `getCloudRequestLog`, `clearCloudRequestLog` are exported from key-vault.ts. Verify `CloudAdapter` has `setPreSendApprovalHandler` and calls it in `execute()` before making the API call.
  </verify>
  <done>
Cloud adapter connects to Anthropic with streaming via SDK and invokes pre-send approval callback before each request (user can cancel). Key vault provides memory-only default + encrypted persistence opt-in. Cloud request log tracks all requests in session. Privacy proxy defines sanitization levels with type boundary enforcement. Per-session consent is tracked. Offline check prevents cloud calls when offline with friendly message. Destructive action guard utility is defined for Phase 5+.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create AI Settings panel, guided setup wizard, cloud request preview, and Shell/StatusBar integration</name>
  <files>
    src/ui/components/AISettingsPanel.tsx
    src/ui/components/AIGuidedSetup.tsx
    src/ui/components/CloudRequestPreview.tsx
...
├── Section: Communication Log (per CONTEXT.md: "Communication log accessible in settings")
│   ├── Header: "Cloud Communication Log" with "Clear" button
│   ├── Show when={getCloudRequestLog().length === 0}: "No cloud requests this session."
│   └── For each log entry: timestamp, provider, status (approved/cancelled/completed/error), truncated prompt preview (expandable)
...
**5. Create `src/ui/components/CloudRequestPreview.tsx`** — Pre-send preview modal:

Per CONTEXT.md locked decision: "every cloud request shows a preview of what the local LLM is sending. User can see exactly what data leaves the device and can cancel before sending."

Props: `{ entry: CloudRequestLogEntry; onApprove: () => void; onCancel: () => void }`

Structure:
```
CloudRequestPreview (modal overlay)
├── Header: "Cloud Request Preview"
├── Info: "The following data will be sent to {entry.provider}. Review before sending."
├── Preview box (scrollable, monospace): entry.sanitizedPrompt
├── Metadata: Provider, Model, Timestamp
├── Footer:
│   ├── [Cancel] button — calls onCancel
│   └── [Send to Cloud] button — calls onApprove
└── Note: "You can disable this preview in AI Settings > Privacy"
```

This component is rendered by Shell.tsx when a cloud request is pending approval. The CloudAdapter's `onPreSendApproval` handler sets a signal in the store that triggers rendering this component.

Add to store.ts (as part of Task 2's scope):
```typescript
// Cloud request preview state
pendingCloudRequest: CloudRequestLogEntry | null;
pendingCloudRequestResolve: ((approved: boolean) => void) | null;
```

**6. Update `src/ui/layout/Shell.tsx`** — Wire AISettingsPanel, AIGuidedSetup, and CloudRequestPreview into the render tree, AND wire the CloudAdapter pre-send approval handler:
...
import { state, setState } from '../signals/store';
import { getCloudAdapter } from '../../ai/router'; // or however the cloud adapter is accessed
```

Add local signal for settings panel visibility:
```typescript
const [showAISettings, setShowAISettings] = createSignal(false);
```

**CRITICAL WIRING — CloudAdapter pre-send approval handler:**

In Shell.tsx's `onMount` (or in a reactive `createEffect` that runs when `cloudAPIEnabled` becomes true), wire the CloudAdapter's `setPreSendApprovalHandler` to the store's `pendingCloudRequest` signal. This is what triggers the `CloudRequestPreview` modal:
...
createEffect(() => {
  if (state.cloudAPIEnabled) {
    const cloudAdapter = getCloudAdapter(); // get the CloudAdapter instance from the router
    if (cloudAdapter) {
      cloudAdapter.setPreSendApprovalHandler((entry) => {
        return new Promise<boolean>((resolve) => {
          ...
          // CloudRequestPreview will now render (reactive on state.pendingCloudRequest)
          // When user clicks Approve/Cancel, resolve(true/false) is called,
          // which unblocks CloudAdapter.execute() and clears the modal
        });
      });
    }
  }
...
This is the key link that connects: CloudAdapter.execute() -> setPreSendApprovalHandler callback -> setState('pendingCloudRequest', ...) -> `<Show when={state.pendingCloudRequest}>` -> CloudRequestPreview renders -> user clicks Approve/Cancel -> resolve(true/false) -> CloudAdapter continues or throws.

Pass the `onOpenAISettings` callback to components that need it. The CommandPalette is already rendered somewhere (likely in Shell or a sibling). Ensure the callback flows through.

Add conditional renders inside the Shell return, after `<StatusBar />`:
```tsx
{/* Phase 4: AI overlays */}
...
<Show when={state.pendingCloudRequest}>
  <CloudRequestPreview
    entry={state.pendingCloudRequest!}
    onApprove={() => {
      state.pendingCloudRequestResolve?.(true);
      setState('pendingCloudRequest', null);
      setState('pendingCloudRequestResolve', null);
    }}
    onCancel={() => {
...
Also expose `setShowAISettings` so CommandPalette's `onOpenAISettings` callback can trigger it. If CommandPalette is rendered inside Shell, pass it as a prop. If CommandPalette is rendered elsewhere, export the signal or use a store field.

Import `getCloudRequestLog` and `clearCloudRequestLog` from `../../ai/key-vault` in AISettingsPanel for the Communication Log section.
  </action>
  <verify>
Run `npx tsc --noEmit` and `pnpm lint`. Verify AISettingsPanel has per-feature toggles for Browser LLM, Cloud API, Triage, Review, Compression. Verify AISettingsPanel has a "Communication Log" section that imports `getCloudRequestLog`. Verify AIGuidedSetup has 4 steps (welcome, local model, cloud API, done). Verify CommandPalette has "AI Settings" command. Verify StatusBar shows AI activity when `state.aiEnabled` is true. Verify Shell.tsx imports and conditionally renders AISettingsPanel, AIGuidedSetup, and CloudRequestPreview. Verify CloudRequestPreview shows `entry.sanitizedPrompt` and has Approve/Cancel buttons.
  </verify>
  <done>
AI Settings panel accessible from Ctrl+P with per-feature toggles, API key management (memory-only default + encrypted persistence), sanitization level control, communication log, and provider status. CloudRequestPreview shows pre-send data preview with cancel option for every cloud request. Guided setup wizard walks through AI enablement on first launch. Shell.tsx renders all AI overlays (settings, guided setup, cloud preview). Status bar shows AI activity indicator. Command palette has "AI Settings" action. All AI features default to disabled (AIST-01).
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: Verify AI settings and guided setup</name>
  <files>src/ui/components/AISettingsPanel.tsx, src/ui/components/AIGuidedSetup.tsx, src/ui/components/CloudRequestPreview.tsx, src/ui/layout/StatusBar.tsx, src/ui/layout/Shell.tsx</files>
  <action>
Human verification checkpoint. What was built: AI Settings panel with per-feature toggles, API key management, communication log, guided setup wizard, cloud request preview modal, status bar AI activity indicator, and Shell.tsx wiring. Cloud adapter with Anthropic streaming and pre-send approval. Privacy proxy and key vault.
  </action>
  <verify>
...
3. AI Settings panel opens:
   ...
   - Scroll to "Communication Log" section — should show "No cloud requests this session."
...
8. Verify Shell.tsx renders AISettingsPanel, AIGuidedSetup, and CloudRequestPreview as overlay components
  </verify>
...
12. Shell.tsx conditionally renders AISettingsPanel, AIGuidedSetup, and CloudRequestPreview
13. CloudRequestPreview shows pre-send data with Approve/Cancel buttons
14. Communication Log section visible in AI Settings with "No cloud requests this session." default
15. Cloud adapter creates log entries and invokes pre-send approval handler before API call
</verification>

<success_criteria>
- Cloud adapter streams responses from Anthropic API with abort support
- Cloud adapter invokes pre-send preview before each request; user can cancel
- Communication log tracks all cloud requests in session (accessible in AI Settings)
...
- Settings panel includes Communication Log section
- CloudRequestPreview modal shows exact sanitized data with Approve/Cancel
- Shell.tsx renders AISettingsPanel, AIGuidedSetup, and CloudRequestPreview as overlays
