...
**6. Update `src/ui/layout/Shell.tsx`** — Wire AISettingsPanel, AIGuidedSetup, and CloudRequestPreview into the render tree:
...
<Show when={state.pendingCloudRequest}>
  <CloudRequestPreview
    ...
    onApprove={() => state.pendingCloudRequestResolve?.(true)}
    onCancel={() => state.pendingCloudRequestResolve?.(false)}
  />
</Show>
```
