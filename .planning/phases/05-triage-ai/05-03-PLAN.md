...
must_haves:
  truths:
    ...
    - "AI responses stream card-by-card per user decision (CONTEXT.md override of AIUX-06 token-by-token requirement — no per-token animation, each card gets a complete suggestion when ready)"
    - "While an item is being processed, its card shows an 'Analyzing...' placeholder state (status: 'pending' in TriageSuggestion with a visual indicator)"
...
- For each item:
  ...
  - Call `onSuggestion` with a **pending placeholder** before the AI call: `{ inboxItemId: item.id, suggestedType: 'fact', suggestedSectionItemId: null, reasoning: '', confidence: 'low', relatedAtomIds: [], status: 'pending' }` — this lets the UI show an "Analyzing..." indicator on the current card while the AI processes it
  ...
  - Parse response, call `onSuggestion` again with updated result (status: 'complete') — this replaces the pending placeholder
  - On parse failure: call `onSuggestion` with status: 'error' and errorMessage
