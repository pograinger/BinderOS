"""
31_train_decomposition_classifier.py -- Train decomposition classifier and export to ONNX.
...
Output:
    ...
    scripts/train/decomposition_test_embeddings.json
    scripts/train/decomposition_python_predictions.json
    scripts/train/decomposition_python_probabilities.json
"""
...
def load_data() -> tuple[list[str], list[str]]:
    ...
    if not input_path.exists():
        ...
        print("Run: python -u scripts/train/30_generate_decomposition_data.py")
        sys.exit(1)
...
def embed_texts(texts: list[str]) -> np.ndarray:
    ...
    # normalize_embeddings=True for browser parity
...
def train_model(X_train: np.ndarray, y_train: np.ndarray):
    ...
    print(f"\n=== Training decomposition classifier ===")
...
def evaluate_model(
    ...
    # Per-class accuracy for worst 5
...
def export_onnx(clf, idx_to_label: dict[int, str]) -> Path:
...
def generate_validation_artifacts(
    ...
    """Generate JSON artifacts for 32_validate_decomposition_model.mjs."""
    import onnxruntime as ort

    test_emb_path = SCRIPT_DIR / "decomposition_test_embeddings.json"
    pred_path = SCRIPT_DIR / "decomposition_python_predictions.json"
    proba_path = SCRIPT_DIR / "decomposition_python_probabilities.json"
...
def main() -> None:
    ...
    # Evaluate
    y_pred, y_proba, overall_acc = evaluate_model(clf, X_test, y_test, idx_to_label)

    # Export ONNX
    onnx_path = export_onnx(clf, idx_to_label)

    # Validation artifacts
    generate_validation_artifacts(X_test, onnx_path)

    # Summary
    onnx_size_kb = onnx_path.stat().st_size / 1024 if onnx_path.exists() else 0
    print("\n" + "=" * 60)
    print("TRAINING COMPLETE -- decomposition")
    print("=" * 60)
    ...
    print(f"\nNext step: node scripts/train/32_validate_decomposition_model.mjs")
    print("=" * 60)
