/**
 * 42_validate_clarification.mjs -- Node.js ONNX validation for clarification classifiers.
 *
 * Uses onnxruntime-node to validate that clarification ONNX model predictions match
 * Python onnxruntime inference at >95% top-1 accuracy.
 *
 * Usage:
 *   node scripts/train/42_validate_clarification.mjs
 *   node scripts/train/42_validate_clarification.mjs --classifier completeness-gate
 *
 * Prerequisites:
...
async function validateClassifier(classifierName, ort) {
  ...
  const embPath = resolve(__dirname, `clarification_${safe}_test_embeddings.json`);
  const predPath = resolve(__dirname, `clarification_${safe}_python_predictions.json`);
  const probaPath = resolve(__dirname, `clarification_${safe}_python_probabilities.json`);
...
async function main() {
  ...
  if (args.includes('--classifier') && args[args.indexOf('--classifier') + 1]) {
    classifiersToValidate = [args[args.indexOf('--classifier') + 1]];
  } else {
  ...
  console.log('=== BinderOS Clarification Classifier Validation ===');
